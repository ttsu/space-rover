...
function parseProgress(raw: string | null): PersistedProgress {
  ...
  try {
    ...
    const appliedUpgrades = Array.isArray(obj.appliedUpgrades) ? (obj.appliedUpgrades as string[]) : []
...
export function getProgress(): PersistedProgress {
  return { ...current, bank: { ...current.bank }, appliedUpgrades: [...current.appliedUpgrades] }
}
...
export function spendFromBank(costs: Partial<Record<ResourceId, number>>): boolean {
